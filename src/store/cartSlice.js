...
export const addItemServer = createAsyncThunk(
    "cart/addItemServer",
    async ({ productId, quantity = 1, setLoader }, { rejectWithValue }) => {
...
export const decreaseItemServer = createAsyncThunk(
    "cart/removeItemServer",
    async ({ productId, setLoader }, { rejectWithValue }) => {
        try {
            setLoader(true);
...
export const removeItemServer = createAsyncThunk(
    ...
    async ({ setLoader }, { rejectWithValue }) => {
        try {
            setLoader(true)
...
export const mergeCart = createAsyncThunk("cart/mergeCart", async (localItems, { rejectWithValue }) => {
    try {
        const payload = localItems.map((it) => ({ product: it.product._id || it.product, quantity: it.quantity }));
        const res = await axios.post("/cart/merge", { items: payload });
        return res.data.cart;
    } catch (err) {
        return rejectWithValue(err.response?.data || err.message);
    }
